...
public class Board {
    ...
    private HashSet<PiecePosition> checkForAvailableMoves(Pieces piece) {
    ...
    public void possibleMoves(String piecePosition) {
        ...
        if (colList.contains(collC) && rowList.contains(rowwC)) {
        ...
        if (board[rowC][colC] != null) {
            HashSet<PiecePosition> availableMoves = checkForAvailableMoves(board[rowC][colC]);
            System.out.print(" \u3000");
            for (char colLabel = 'a'; colLabel <= 'h'; colLabel++) {
                System.out.printf("  " + colLabel + "\u3000");
            }
            ...
            for (int row = 0; row < 8; row++) {
                ...
                for (int col = 0; col < 8; col++) {
                    PiecePosition currentMove = new PiecePosition(row, col);

                    System.out.print("|");
                    if (availableMoves.contains(currentMove)) {
                        System.out.print(" ▢ ");
                    } else {
                        System.out.print(" \u3000 ");
                    }
            ...
            System.out.println();
        } else System.out.println("Não existe nenhuma peça nesse lugar.");
    }
    ...
    public void movePiece(String piecePosition, String targetedPosition) {
        ...
        if (colList.contains(collC) && rowList.contains(rowwC) && colList.contains(collT) && rowList.contains(rowwT)) {
            ...
            rowT = 8 - Integer.parseInt(String.valueOf(rowwT));
            switch (collT) {
                case 'a' -> colT = 0;
                case 'b' -> colT = 1;
                case 'c' -> colT = 2;
            ...
            Pieces currentPiece = board[rowC][colC];
            HashSet<PiecePosition> availableMoves = checkForAvailableMoves(currentPiece);

            if (availableMoves.contains(PP)) {
                if (currentPiece instanceof King || currentPiece instanceof Rook || currentPiece instanceof Pawn) {
                    currentPiece.setHasMoved();
                }
                board[rowT][colT] = currentPiece;
